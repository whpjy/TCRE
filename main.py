...
def evaluate(test_batch, rel2idx, ner2idx, args, test_or_dev):
    ...
    with torch.no_grad():
        for data in test_batch:
            ...
            ner_pred, re_pred, loss_f = model(text, mask)
            test_loss = test_loss + loss + loss_f
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data", default="SCIERC", type=str,
    ...
    parser.add_argument("--embed_mode", default="scibert", type=str,
    ...
    if args.do_train:
        ...
        model = TCRE(args, input_size, ner2idx, rel2idx)
        model.to(device)
        ...
        for epoch in range(args.epoch):
            steps, train_loss, train_loss_ner ,train_loss_re = 0, 0, 0, 0
            ...
            for data in tqdm(train_batch):
                ...
                ner_pred, re_pred, loss_f = model(text, mask)
                ner_loss, re_loss = BCEloss(ner_pred, ner_label, re_pred, re_label)

                if len(hisstry_loss_ner) > 2:
                    if hisstry_loss_ner[-3] == 0 or hisstry_loss_re[-3] == 0:
                        loss = ner_loss + re_loss
                    else:
                        t_e =  hisstry_loss_ner[-2] / hisstry_loss_ner[-3]
                        t_r = hisstry_loss_re[-2] / hisstry_loss_re[-3]
                        w_e = 2 * math.exp(t_e) / (math.exp(t_e) + math.exp(t_r))
                        w_r = 2 * math.exp(t_r) / (math.exp(t_e) + math.exp(t_r))
                        loss = w_e * ner_loss + w_r * re_loss
                else:
                    loss = ner_loss + re_loss
                ...
                train_loss_ner += ner_loss.item()
                train_loss_re += re_loss.item()
            ...
            hisstry_loss_ner.append(train_loss_ner / steps)
            hisstry_loss_re.append(train_loss_re / steps)
